...
<body>
	<ul class="for_ul_global">
		...
		<div style="margin: 20px 0 0 0;">
			...
			<li><a class="for_a_global" href="user?type=sta&type_sta=sta&userid=${userid}">统计</a></li>
	...
	<div class="container" style="height:800px;">
		<form action="wardrobe?type=search&type_s=all&pageIndex=1&userid=${userid}" role="form" method="post">
			<div class="same">
		...
		<div style="margin-top:40px;height:700px;border-style:dashed;border-width:5px 5px;border-color: lightpink;border-radius: 0.5em;">
			<div style="margin:20px 10px;">
				<ul class="for_wardrobe_ul">
					<li><a ${"id".equals(type_s)?"class='active'":""} href="wardrobe?type=search&pageIndex=${pageIndex}&userid=${userid}&key=${key}&type_s=id">id</a></li>
					<li><a ${"name".equals(type_s)?"class='active'":""} href="wardrobe?type=search&pageIndex=${pageIndex}&userid=${userid}&key=${key}&type_s=name">名称</a></li>
					<li><a ${"type".equals(type_s)?"class='active'":""} href="wardrobe?type=search&pageIndex=${pageIndex}&userid=${userid}&key=${key}&type_s=type">类型</a></li>
				</ul>
				...
				<tr><td colspan="8" align="center">
					<div class="pager">
						<ul class="clearfix">
							<li><a href="wardrobe?type=search&pageIndex=${param.pageIndex-1}&type_s=${type_s}&userid=${userid}&key=${key}">上一页</a></li>
							<c:forEach var="i" begin="1" end="${pageCount}" step="1">
								<c:if test="${i==param.pageIndex}">
									<li class="current"><a href="wardrobe?type=search&pageIndex=${i}&type_s=${type_s}&userid=${userid}&key=${key}">${i}</a></li>
								</c:if>
								<c:if test="${i!=param.pageIndex}">
									<li><a href="wardrobe?type=search&pageIndex=${i}&type_s=${type_s}&userid=${userid}&key=${key}">${i}</a></li>
								</c:if>
							</c:forEach>
							<li><a href="wardrobe?type=search&pageIndex=${param.pageIndex+1}&type_s=${type_s}&type_q=${type_q}&userid=${userid}&key=${key}">下一页</a></li>
						</ul>
					</div>
				</td></tr>
